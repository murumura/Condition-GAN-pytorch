        ...
        with tqdm(total=n_train, desc=f'Epoch {epoch + 1}/{epochs}', unit='img') as pbar:
            for batch_idx, (data, target) in enumerate(self.data_loader):
                ...
                if verbose and batch_idx % (log_interval) == 0 and batch_idx > 0:
                    pbar.set_postfix(**{'Discriminator loss (batch)': d_loss.mena().item()})
                    pbar.set_postfix(**{'Generator loss (batch)': g_loss.mean().item()})
                    pbar.set_postfix(**{'time:': time.time() - batch_time})
